 ...
 *
 *			Mods by Dave Mudgett 08/30/2015 allow the user to also
 *			include a filename to open as a command-line argument,
 *			plus cleaned up source code wrt to indenting and 
 * 			bracketing if/else blocks.
 *			
 *
...
private JMenuBar k;
	...
	*/
	...
	/**
	* Create a TrivialEdit window, with a JTextArea where the user can
	* edit some text and with a menu bar, and open up the file given
	...
	*/
	...
	{     
		...
		if (selectedFile.exists()) // Ask the user whether to replace the file.
		{
			...
			if (response != JOptionPane.YES_OPTION) {return;}  // User does not want to replace the file.
		}
		PrintWriter out; 
		try 
		{
   ...
   /**
    * Open the file specified by the user in a JFileChooser dialog and 
    * read up to 10000 characters from that file.  If the file is read 
    * successfully, then the text from the file replaces the 
...
//				throw new IOException("Input file is too large for this program.");
    ...
    * program and read up to 10000 characters from that file if it's 
    * opened successfully.
    */
